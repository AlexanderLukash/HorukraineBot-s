...
class Levelsys(commands.Cog):
        ...
        if not message.content.startswith("?"):
            ...
            if not message.author.bot:
                    ...
                    if str(message.author.id) in data[str(message.guild.id)]:
                        ...
                        if new_level > lvl:
                            await message.channel.send(
                                f"{message.author.mention} підвищів свій рівень до {new_level}!!!")
                            ...
                            for i in range(len(level)):
                                if new_level == level_num[i]:
                                    ...
                                    mbed = discord.Embed(title=f"{message.author} Ви отримали роль **{level[i]}**!",
                ...
                if str(message.author.id) in user_data:
                    ...
                    user_data[str(message.author.id)]['text_color'] = "#d8a0fe"
                    user_data[str(message.author.id)]['bar_color'] = "#d8a0fe"
    ...
    async def leader(self, ctx, range_num=5):
        await ctx.channel.purge(limit=1)
        with open("levels.json", "r") as f:
            data = json.load(f)
        ...
        for amt in total_xp:
            ...
            if member is not None:
                ...
                if index == range_num:
                    ...
                    index += 1
        mbed.set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url)
    ...
    async def rank_reset(self, ctx, user: Optional[discord.Member]):
        ...
        if not member == ctx.author:
            ...
            if not role in member.roles:
                await ctx.author.send(f"Ви можете скинути лише свої дані, щоб скинути інші дані, які ви повинні мати {role} роль.")
                return
        ...
        await ctx.send(f"{member.mention} дані було скинуто.")
    ...
    async def increase_level(self, ctx, increase_by: int, user: Optional[discord.Member]):
        ...
        await ctx.send(f"{member.mention}, Ваш рівень був підвищений на {increase_by}")
    ...
    async def increase_xp(self, ctx, increase_by: int, user: Optional[discord.Member]):
        ...
        await ctx.send(f"{member.mention}, Ваш досвід був збільшений на {increase_by}")
